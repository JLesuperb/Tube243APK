package com.module.search.views;
...
import com.module.search.adapters.SearchAdapter;
import com.module.search.arrows.ArrowDrawable;
import java.util.List;

/**
 * Created by JonathanLesuperb on 2018/05/06.
 */
...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener
{
    ...
    private MaterialSearchView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    {
        TypedArray attr = mContext.obtainStyledAttributes(attrs, R.styleable.MaterialSearchView, defStyleAttr, defStyleRes);
        if (attr != null) {
            if (attr.hasValue(R.styleable.MaterialSearchView_search_version))
            {
                setVersion(attr.getInt(R.styleable.MaterialSearchView_search_version, SearchCodes.VERSION_TOOLBAR));
            }
            if (attr.hasValue(R.styleable.MaterialSearchView_search_style))
            {
                setStyle(attr.getInt(R.styleable.MaterialSearchView_search_style, SearchCodes.STYLE_TOOLBAR_CLASSIC));
            }
            if (attr.hasValue(R.styleable.MaterialSearchView_search_theme))
            {
                setTheme(attr.getInt(R.styleable.MaterialSearchView_search_theme, SearchCodes.THEME_LIGHT));
            }
            ...
            if (attr.hasValue(R.styleable.MaterialSearchView_search_hint_size))
            {
                setHintSize(attr.getDimensionPixelSize(R.styleable.MaterialSearchView_search_hint_size, 0));
            }
            ...
            {
                setAnimationDuration(attr.getInt(R.styleable.MaterialSearchView_search_animation_duration, ANIMATION_DURATION));
            }
            attr.recycle();
        }
    }
